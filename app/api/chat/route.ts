...
const roles: Record<string,{name:string;model:string;instructions:string}> = {
  elir:{name:'Элир',model:'gpt-5.6-sol',instructions:'Ты Элир — главный личный AI-помощник и координатор команды. Отвечай по-русски, тепло, конкретно и без лишней воды. Если задача требует узкой экспертизы, обозначай нужного специалиста.'},
...
const scopePrompt:Record<string,string>={
  'Личное':'Контекст приватный. Не переносить детали в семейный или рабочий контекст без явного решения пользователя.',
  'Семья':'Контекст общий семейный. Ответ должен быть понятен участникам семьи и учитывать общие договорённости.',
...
export async function POST(req:Request){
  try{
    ...
    const body=await req.json(); const chosen=roles[body.agent]||roles.elir
    ...
    if(body.council){
      ...
      const notes=await Promise.all(council.map(async id=>{
        const r=roles[id]
        const out=await client.responses.create({model:r.model,instructions:`${r.instructions}\n${scopePrompt[body.scope]||''}\nДай внутреннюю записку Элиру: максимум 5 коротких пунктов.`,input:body.message})
        return `${r.name}: ${out.output_text}`
      }))
      const final=await client.responses.create({model:'gpt-5.6-sol',instructions:`${roles.elir.instructions}\n${scopePrompt[body.scope]||''}\nСобери единый ответ пользователю на основе мнений специалистов. Не изображай консенсус, если мнения расходятся. В конце дай конкретные следующие действия.`,input:`Запрос пользователя: ${body.message}\n\nМнения совета:\n${notes.join('\n\n')}`})
    ...
    const out=await client.responses.create({model:chosen.model,instructions:`${chosen.instructions}\n${scopePrompt[body.scope]||''}`,input:`История:\n${history}\n\nОтветь на последнее сообщение пользователя.`})
    return NextResponse.json({agent:chosen.name,text:out.output_text})
  }catch(e:any){return NextResponse.json({error:e?.message||'Ошибка AI-сервера'},{status:500})}
}