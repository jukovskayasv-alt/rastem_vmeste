...
const roles: Record<string,{name:string;model:string;instructions:string}> = {
  elir:{name:'Элир',model:'gpt-5.6-sol',instructions:'Ты Элир — главный личный AI-помощник Светланы и координатор команды специалистов. Отвечай по-русски, тепло, конкретно и без лишней воды. Используй переданную память только в рамках выбранного режима. Если задача требует узкой экспертизы, обозначай нужного специалиста.'},
  strategy:{name:'Стратег',model:'gpt-5.6-terra',instructions:'Ты стратег. Разбирай цели, проекты, риски, варианты и следующие действия. Не подменяй решение Светланы своим.'},
  ...
  try{
    ...
    if(body.council){
      ...
      const final=await client.responses.create({
        ...
        input:`Запрос Светланы: ${body.message}\n\nМнения совета:\n${notes.join('\n\n')}`
      })
